...
#[derive(Debug, Default)]
struct RayTracingPipelineInfoBuf {
    shader_groups: Vec<ComputePipelineInfoBuf>,
}
impl RayTracingPipelineInfoBuf {
    unsafe fn initialize<'a>(this: &mut Pin<&mut Self>, desc: &pso::RayTracingPipelineDesc<'a, B>) {
        let this = Pin::get_mut(this.as_mut()); // use into_inner when it gets stable
...
impl d::Device<B> for Device {
    ...
    unsafe fn create_ray_tracing_pipeline<'a>(
        ...
        let mut buf = RayTracingPipelineInfoBuf::default();
        let mut buf = Pin::new(&mut buf);
        RayTracingPipelineInfoBuf::initialize(&mut buf, desc);
